            ...
            else
            {
                if (in_array($vpb_file_extensions, $vpb_allowed_extensions)) 
                {
                    ...
                    for ($iCnt = 0; $iCnt < sizeof($hakAndHakgwaAndClasArrayList); $iCnt++) {
                        if (preg_match("/과/", $hakAndHakgwaAndClasArrayList[$iCnt]) || preg_match("/일반/", $hakAndHakgwaAndClasArrayList[$iCnt]) || preg_match("/계열/", $hakAndHakgwaAndClasArrayList[$iCnt])) {
                            $__hakAndHakgwaAndClasArrayList =  explode("(",$hakAndHakgwaAndClasArrayList[$iCnt]);
                            $gyeol = trim($__hakAndHakgwaAndClasArrayList[0]);	// 학과 추출
                        }
                
                        if (preg_match("/학년/", $hakAndHakgwaAndClasArrayList[$iCnt])) $grade = preg_replace("/[^0-9]*/s", "", $hakAndHakgwaAndClasArrayList[$iCnt]); // 학년
                    ...
                    if ($gyeol == "" || $grade == "") {
						...
						$selrst = $db->query("SELECT std_unique, gyeol, name FROM member WHERE std_pyear = '" . $syrw['GRADEYEAR'] . "' AND hak = '" . $grade . "' AND gyeol = '" . $gyeol . "' AND use_yn = 'Y';");
                            ...
                            foreach ($allData as $key => $value) {
								...
								if ($excelBan != '' && $excelBun != '' && $excelName != '') {
									...
									if ($excelBan != '반' && $excelBun != '번호' && $excelName != '성명' && sizeof($itemInfoArray) > 0) {	// 이런 항목이 있다면 넣지 말고
										foreach ($itemInfoArray AS $ukey => $uValue) {
											// echo $itemCodeArray[$key] . "<br>";
											$studentInfoArray[(int)$excelBan][(int)$excelBun][$excelName][$itemCodeArray[$ukey]] = iconv("UTF-8", "EUC-KR", $value[$uValue]);
											if ($itemCodeArray[$ukey] == 'ONLYNUMBER') $distinctArray['s' . iconv("UTF-8", "EUC-KR", $value[$uValue])] = 'TRUE';	// 중복아이디 필터링 기능
										}
									}
								}								
