            ...
            else
            {
                if (in_array($vpb_file_extensions, $vpb_allowed_extensions)) 
                {
                    ...
                    for ($iCnt = 0; $iCnt < sizeof($hakAndHakgwaAndClasArrayList); $iCnt++) {
                        if (preg_match("/과/", $hakAndHakgwaAndClasArrayList[$iCnt]) || preg_match("/일반/", $hakAndHakgwaAndClasArrayList[$iCnt]) || preg_match("/과정/", $hakAndHakgwaAndClasArrayList[$iCnt]) || preg_match("/계/", $hakAndHakgwaAndClasArrayList[$iCnt])) {	
                        	// 과를 공통과정(전문계) 이런식으로 해서 필여없음
...
//                            $nHakgwa = trim($__hakAndHakgwaAndClasArrayList[0]);	// 학과 추출
                            $nHakgwa = trim($hakAndHakgwaAndClasArrayList[$iCnt]);	// 학과 추출
                    ...
                    if ($nHakgwa == '' || $nHak == '' || $nBan == '') {
						...
						$selrst = $db->query("SELECT std_unique, gyeol, name FROM member WHERE std_pyear = '" . $syrw['GRADEYEAR'] . "' AND hak = '" . $nHak . "' AND gyeol = '" . $nHakgwa . "';");
						$tmpMyinfo = array();
